...
## Account-Specific Storage Architecture

For add-ons with identity/account-specific templates or settings, implement a **three-domain storage pattern** to organize data clearly and enable selective export/import.

### Storage Domains

**1. `settings` domain** — General add-on preferences (101+ settings)
- Core functionality toggles
- UI preferences
- File paths, formatting options
- License information
- Stored at: `browser.storage.local.settings`

**2. `debug` domain** — Debug flags (40+ flags)
- Global debug switch: `debugActive`
- Component-specific flags: `debug.composer`, `debug.mime`, etc.
- Stored at: `browser.storage.local.debug`

**3. `accounts` domain** — Identity-specific templates
- Per-identity settings keyed by identity ID
- Pattern: `id1.new.html`, `id1.rsp.header`, `common.signature`
- Stored at: `browser.storage.local.accounts`

### Account Storage Pattern

**Legacy prefs pattern:**
```
extensions.smartTemplate4.id1.new.html
extensions.smartTemplate4.id1.rsp.header
extensions.smartTemplate4.id2.fwd.msg
extensions.smartTemplate4.common.signature
```
...
### Known Account Settings

For SmartTemplates, 17 settings per identity:
- `def` — default template flag
- **Forward:** `fwd`, `fwdhead`, `fwdheader`, `fwdhtml`, `fwdmsg`, `fwdnbr`
- **New/Write:** `new`, `newhtml`, `newmsg`, `newnbr`
- **Reply:** `rsp`, `rsphead`, `rspheader`, `rsphtml`, `rspmsg`, `rspnbr`

### Migration Strategy

**DO NOT migrate all possible account keys blindly.** Only migrate accounts with actual stored data:

```javascript
async _migrateLegacyPrefs() {
  const accounts = {};
  const identities = await messenger.accounts.list().then(
    accts => accts.flatMap(a => a.identities || [])
  );
  
  // FIRST PASS: Check which accounts have ANY stored settings
  const accountsWithData = new Set();
  const knownKeys = ['def', 'fwd', 'fwdhead', /* ... all 17 keys */ ];
  
  for (const identity of identities) {
    const accountId = identity.id; // Already returns "id1", "id2" format
    const hasData = knownKeys.some(key => 
      legacyPrefs.has(`extensions.smartTemplate4.${accountId}.${key}`)
    );
    if (hasData) {
      accountsWithData.add(accountId);
    }
  }
  
  // SECOND PASS: Only migrate accounts in accountsWithData
  for (const accountId of accountsWithData) {
    accounts[accountId] = {};
    for (const key of knownKeys) {
      const prefName = `extensions.smartTemplate4.${accountId}.${key}`;
      if (legacyPrefs.has(prefName)) {
        accounts[accountId][key] = legacyPrefs.get(prefName);
      }
    }
  }
  
  return { settings, debug, accounts };
}
```

**CRITICAL:** `identity.id` already returns `"id1"`, `"id2"` format. Do NOT prefix with extra `"id"` or you'll create `"idid1"`.

### Account API Methods

Add these methods to your Preferences module:

```javascript
// Get single account setting
getAccountPref(accountId, key) {
  return this._accountsData[accountId]?.[key];
}

// Set single account setting
async setAccountPref(accountId, key, value) {
  if (!this._accountsData[accountId]) {
    this._accountsData[accountId] = {};
  }
  this._accountsData[accountId][key] = value;
...
// Get all settings for one account (for UI display)
getAccountSettings(accountId) {
  return { ...this._accountsData[accountId] };
}

// Export all accounts (for backup/export feature)
exportAccountSettings() {
  return JSON.parse(JSON.stringify(this._accountsData));
}

// Import all accounts (for restore/import feature)
async importAccountSettings(accountsData) {
  this._accountsData = accountsData;
  await browser.storage.local.set({ accounts: accountsData });
}
```

### Benefits of Three-Domain Architecture

1. **Clean separation** — Settings, debug flags, and account templates don't mix
2. **Selective export** — Users can backup just account templates
3. **Easier reset** — Clear one domain without affecting others
4. **Better organization** — storage.local keys map to logical feature groups
5. **Efficient migration** — Only migrate accounts users actually customized
...
## SmartTemplates specific storage considerations
