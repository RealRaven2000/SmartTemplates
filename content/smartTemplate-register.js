...
SmartTemplate4.Licenser = {
  ...
  get isValidated() {
	...
	get isExpired() {
		let key = SmartTemplate4.Preferences.getStringPref('LicenseKey');
		if (!key) return false;
  ...
  showDialog: function showDialog(featureName) {
		...
		SmartTemplate4.Util.Mail3PaneWindow.openDialog('chrome://SmartTemplate4/content/register.xul','smarttemplate4-register','chrome,titlebar,centerscreen,resizable,alwaysRaised,instantApply',SmartTemplate4,params).focus();
  } ,
  ...
  load: function load() {
    const getElement = document.getElementById.bind(document),
          util = SmartTemplate4.Util,
					licenser = SmartTemplate4.Util.Licenser,
    ...
    if (decryptedDate) {
			...
			if (licenser.ValidationStatus == ELS.NotValidated) {
				licenser.validateLicense(SmartTemplate4.Preferences.getStringPref('LicenseKey'));
      ...
      getElement('licenseDate').value = decryptedDate; // invalid ??
			if (licenser.isExpired) {
				...
				btnLicense.setAttribute('oncommand', 'SmartTemplate4.Licenser.goPro(2);');
			}
		}
    else
  ...
  goPro: function goPro(license_type) {
    ...
    switch	(license_type) {
			...
			case 1: // domain license
			  alert("Sorry - Domain licenses for SmartTemplate4 are still under development. Please contact me directy to discuss this.");
				shortOrder = "http://sites.fastspring.com/quickfolders/instant/smarttemplate4domain";
			...
			case 2: // license renewal
				...
				// addQuery = "&renewal=" + encodeURI(SmartTemplate4.Preferences.getStringPref('LicenseKey'));
				featureName = encodeURI(SmartTemplate4.Preferences.getStringPref('LicenseKey'));
  ...
  premiumInfo: function premiumInfo(event) {
    SmartTemplate4.Util.openURL(event,'http://smarttemplate4.mozdev.org/premium.html');
  },
  ...
  getDate: function getDate(LicenseKey) {
    ...
    if (!arr.length) {
      SmartTemplate4.Util.logDebug('getDate() failed - no ; found');
      return ''; 
    }
    // get date portion
  ...
  getMail: function getMail(LicenseKey) {
    ...
    if (!arr1.length) {
      SmartTemplate4.Util.logDebug('getMail() failed - no : found');
      return '';
    }
  ...
  getCrypto: function getCrypto(LicenseKey) {
    let arr=LicenseKey.split(';');
    if (arr.length<2) {
      SmartTemplate4.Util.logDebug('getCrypto() failed - no ; found');
      return null;
    }
    return arr[1];
  },
  ...
  validateLicense: function validate(LicenseKey) {
    ...
    if (LicenseKey.indexOf('STD')==0) {
      ...
      SmartTemplate4.Crypto.key_type=0;
    }
    let maxDigits = SmartTemplate4.Crypto.maxDigits, // will be 67 for Domain License
    ...
    for (let a=0; a < myAccounts.length; a++) { 
      ...
      if (ac.defaultIdentity && !ForceSecondaryMail) {
        ...
        if (!this.AllowSecondaryMails
            ||  
            SmartTemplate4.Crypto.key_type == 1) 
          continue;  
  ...
  encryptLicense: function encryptLicense(LicenseKey, maxDigits) {
    SmartTemplate4.Util.logDebug ('encryptLicense - initialising with maxDigits = ' + maxDigits);
    SmartTemplate4.RSA.initialise(maxDigits);
    // 64bit key pair
    SmartTemplate4.Util.logDebug ('encryptLicense - creating key pair object, bit length = ' + this.RSA_keylength);
    ...
    SmartTemplate4.Util.logDebug ('encryptLicense - starting encryption...');
    let Encrypted = SmartTemplate4.RSA.encryptedString(key, LicenseKey, 'OHDave');
    SmartTemplate4.Util.logDebug ('encryptLicense - finished encrypting registration key of length: ' + Encrypted.length + '\n'
      + Encrypted);
    return Encrypted;
