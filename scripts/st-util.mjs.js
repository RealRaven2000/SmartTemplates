...
export let Util = {
  ...
  logWithOption: function logWithOption() {
    arguments[0] =
      "SmartTemplates " + "{" + arguments[0].toUpperCase() + "} " + Util.logTime() + "\n";
    console.log(...arguments);
  },
  ...
  logWarning: function (_a) {
    let msg = "SmartTemplates " + this.logTime() + "\n";
  ...
  wrapDeferredHeader: async function wrapDeferredHeader(field, defaultValue, isHtml, isComposeNew) {
    ...
    let tag =
      `<smarttemplate hdr='${generalFunction}' st4variable='${field}' title='${field}' ${newComposeClass}>` +
      defaultValue +
      "</smarttemplate>";
  ...
  prTime2Str: async function (time, timeType, timezone, offsets) {
    ...
    async function getDateFormat(field) {
      return await messenger.LegacyPrefs.getPref("extensions.smartTemplate4.dateformat." + field);
    }
    ...
    try {
      ...
      Util.logDebugOptional(
        "timeStrings",
        "DateTimeFormat(" + localeString + ") resolved options: " + fmt.resolvedOptions()
      );
      ...
      if (forceTimeZone) {
        let forceHours = this.getTimezoneOffset(forceTimeZone);
